...
function ban() {
    let capital = parseFloat(prompt('Ingrese el valor que depositó al banco:'));
    let años = parseInt(prompt('¿A cuántos años desea invertir?'));
    ...
    let interesMensual = 0.7 / 100; // Convertimos el porcentaje en decimal
    let meses = años * 12; // Convertimos los años a meses

    // Fórmula para calcular el monto final con interés compuesto
    let saldoFinal = capital * Math.pow(1 + interesMensual, meses);

    alert(`Después de ${años} años, su saldo será: ${saldoFinal.toFixed(2)} unidades monetarias.`);
}
...
function NumerosEnRango() {
    ...
    let resultado = 'Los números en el rango son:\n';

    // Generar números dentro del rango
...
function AreaCirculo() {
    ...
    if (isNaN(radio) || radio <= 0) {
        alert('Por favor, ingrese un radio válido (mayor que 0).');
        return;
    }

    let area = Math.PI * Math.pow(radio, 2);

    alert(`El área del círculo con radio ${radio} es ${area.toFixed(2)} unidades cuadradas.`);
}
